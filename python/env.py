...
class EnvironmentGroup:
    ...
    def get_actions(self, device: torch.device) -> Actions:
        ...
        return Actions(
            torch.from_numpy(room_idx).to(device),
            torch.from_numpy(room_x).to(device),
            torch.from_numpy(room_y).to(device),
        )
    ...
    def get_outcomes(self, device: torch.device) -> Outcomes:
        ...
        return Outcomes(
            torch.from_numpy(door_invalid).to(device),
            torch.from_numpy(connection_invalid).to(device),
        )
